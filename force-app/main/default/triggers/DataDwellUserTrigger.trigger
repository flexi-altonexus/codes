trigger DataDwellUserTrigger on User (before insert, after insert, before update, after update) {
    ...
    if(Trigger.isAfter) {
        ...
        List<String> permissionSetNames = new List<String>{'Sales_Enablement_Basic', 'Sales_Enablement_Flexioffices'};
        String publicGroupName = 'Data_Dwell_SE';
        ...
        if(usersAdd.size() > 0 || usersRemove.size() > 0) {
            ...
            List<Group> groupList = [SELECT Id FROM Group WHERE DeveloperName = :publicGroupName];
            if(groupList.size() == 1) {
